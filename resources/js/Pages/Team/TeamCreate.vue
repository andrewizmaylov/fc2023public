        ...
        <FormHeader :model="MODEL"
            ...
            <section class="flex justify-between">
                <ImageBlock :input="input"
                            ...
                            :setup="image_data"/>
                <section>
                    <span class="btn_main btn_ld cursor-pointer"
                          style="height: 26px;"
                          @click="toggleClub(input.id, 'Team')">{{ input.club_id ? $t('member') : $t('foreign') }}</span>
                    <p class="text_12 mt-2 opacity-80"
                       v-html="$t('member_message')"/>
                </section>
            </section>
        </FormHeader>
...
import {toggleClub} from '@/Pages/Shared/helpers.js';

const { t } = useI18n();
const props = defineProps(['input']);
const image_data = {
    css: 'relative w-[400px] h-[300px] rounded-[10px] ringed_10 overflow-hidden bg-gray-100',
    field: 'logo',
};
...
onMounted(() => {
    ...
    Event.on('club_toggled', () => router.reload({only: ['input']}));
