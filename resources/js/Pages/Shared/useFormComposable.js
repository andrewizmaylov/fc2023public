...
export default (model) => {
	...
	function updateForm(form, model) {
		// console.log('composable updateForm', form, model, model_route(model), model_route(model.toLowerCase()));
		
		form.put(route(model_route(model.toLowerCase()) + '.update', {slug: form.slug, id: form.id}), {
		// form.patch(route(model_route(model.toLowerCase()) + '.update', {slug: form.slug}), {
			errorBag: 'updateInformation',
			preserveScroll: true,
			onSuccess: () => {
	...
	const model_route = (model) => {
		return ['news', 'merch'].includes(model.toLowerCase()) ? model : model + 's';
	};
